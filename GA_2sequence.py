...
class DSA:
    def __init__(self, seq1, seq2, CROSSOVER_RATE=0.8, MUTATION_RATE=0.01, N_GENERATIONS=50):
        # 参数初始化
        self.seq_1 = seq1
        self.seq_2 = seq2
        ...
        self.size1 = len(seq1)
        self.size2 = len(seq2)
        self.size = self.size1 + self.size2  # 每个个体的基因长度为两条序列相加
        # 初始化种群
        self.pop = np.random.randint(3, size=(self.popsize, self.size))
    ...
    def mutation(self, child):
        if np.random.rand() < self.mutation_rate:  # 以MUTATION_RATE的概率进行变异
            ...
            child[mutate_point] = np.random.randint(3)

    def select(self):    # 自然选择函数，采用轮盘赌策略
        fitness = self.fitness()
        #max_element = max(fitness)
        # fitness = np.array([(max_element-value)*100+0.01 for value in fitness])   #概率的替代策略1
        fitness = np.array([1/value for value in fitness])
        # 归一化适应度函数  概率的替代策略2
        #min_element = min(fitness)
        #fitniss_normal = np.array([(value-min_element)/(max_element-min_element) for value in fitness])
        # probability = 1-np.exp(-fitness_normal)     # 采用 1-e^-1作为概率函数
...
if __name__ == "__main__":
    ...
    file_handle = open('GA_result2.txt', mode='a')  # 保存结果

    for idx, seq_1 in enumerate(sample_lines):
        score = []

        for seq_2 in database_lines:
            dsa = DSA(seq_1, seq_2, CROSSOVER_RATE=0.8)
            score.append(dsa.generation())
