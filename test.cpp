...
{
public:
    Astar(string _seq1, string _seq2, string _seq3);
...
int Astar::calcH(Point *point)
{
    ...
    int min,mid,max;
    //比较d_x,d_y,d_z大小
	if (d_x <= d_y) {min = d_x; mid = d_y;}
    else {min = d_y; mid = d_x;}
    if (d_z < min) {max = mid; mid = min; min = d_z;}
    else{
        if(d_z >= mid) max = d_z;
        else {max = mid;mid = d_z;}
    }
    */
    return 2 * GAPCOST * (max(d_x, max(d_y, d_z)) - min(d_x, min(d_y, d_z))) 
        + 3*0.5* MATCHCOST*(min(d_x, min(d_y, d_z)));
}
...
Point *Astar::findPath(Point &startPoint)
{
    ...
    while (!openList.empty())
    {
        ...
        for (auto &target : surroundPoints)
        {
            ...
            if(tempF > (curPoint->F+5)&& (curPoint->F!=0))  {   //采用部分扩展
                //cout<<tempF<<endl;
                continue;}
...
bool Astar::isCanreach(const Point *point, const Point *target) const
{
    if (target->x > col || target->y > row || target->z > high ||
        ...
        graph[target->z][target->y][target->x] == true
        ) //如果点与当前节点重合、超出地图、在close表返回false
        return false;
    else
        return true;
}
...
int main()
{
    ...
    double time1 = 0;
    Start = clock();
    string seq1 = "IPZJJLMLTKJULOSTKTJOGLKJOBLTXGKTPLUWWKOMOYJBGALJUKLGLOSVHWBPGWSLUKOBSOPLOOKUKSARPPJ";
    string seq2 = "IPJTUMAOULBGAIJHUGBSOWBWLKKBGKPGTGWCIOBGXAJLGTWCBTGLWTKKKYGWPOJL";
    string seq3 = "BNIPMBSKHSASLLXKPIPLPUVHKHCSJCYAPLUKJGSGPGSLKUBDXGOPKLTLUCWKAUSL";
    Point start(0, 0, 0);
    Astar astar(seq1,seq2,seq3);
    int score = astar.findPath(start)->G;
    cout << score<<endl;
    endt = clock();
    time1 = (double)(endt - Start) / CLOCKS_PER_SEC;
